...
public class DataSetProviderShouldMatchDataSetTest
{
   ...
   public void shouldThrowExceptionWhenFormatCannotBeInferedFromFileExtension() throws Exception
   {
      ...
      // when
      List<Format> expectedDataFormats = dataSetProvider.getDataFormats(testEvent.getTestMethod());
   ...
   public void shouldProvideDefaultFileNameWhenNotSpecifiedInAnnotationOnClassLevel() throws Exception
   {
      ...
      // when
      Set<DataSetDescriptor> files = dataSetProvider.getDescriptors(testEvent.getTestClass());

      // then
      assertThat(files).containsOnly(new DataSetDescriptor(expectedFileName, Format.EXCEL));
   }
