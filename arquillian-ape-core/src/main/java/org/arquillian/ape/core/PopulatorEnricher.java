...
public abstract class PopulatorEnricher<T extends PopulatorService> implements ResourceProvider {
    ...
    public Object lookup(ArquillianResource resource, Annotation... qualifiers) {
        ...
        if (!populatorAnnotation.isPresent()) {
            throw new IllegalArgumentException(
                "Element is annotated with Arquillian Resource of type Populator, but there is no annotation meta-annotated witt @Populator for choosing implementation.");
        }

        final Optional<T> populatorService = findPopulatorService(populatorAnnotation.get());

        if (!populatorAnnotation.isPresent()) {
            throw new IllegalArgumentException(String.format("No Populator Service found for annotation %s.",
                populatorAnnotation.get().annotationType()));
        }
    ...
    /**
     * Method that instantiate the custom populator DSL specific for each case. For example NoSql populator, Sql
     * populator, ...
     *
     * @param populatorService
     *     used in DSL Populator to execute and clean methods.
     *
     ...
     *
     * @param annotation
     *     that specifies the populator service.
     *
     ...
     *
     * @param qualifiers
     *     present in Arquillian Resource
     *
    ...
    private Optional<Annotation> getPopulatorAnnotation(Annotation... qualifiers) {
        return Arrays.stream(qualifiers)
            .filter(annotation -> annotation.annotationType().getAnnotationsByType(org.arquillian.ape.api.Populator.class)
                != null)
            .findFirst();
    }
}
