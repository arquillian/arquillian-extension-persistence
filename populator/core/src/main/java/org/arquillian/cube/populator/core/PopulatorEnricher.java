...
public abstract class PopulatorEnricher<T extends PopulatorService> implements ResourceProvider {
    ...
    /**
     * Method that finds which annotation is meta-annotated with org.arquillian.cube.populator.api.Populator.
     * In case of multiple annotations found, the first one is returned.
     *
     * @param qualifiers present in Arquillian Resource
     * @return Annotation found meta-annotated with org.arquillian.cube.populator.api.Populator
     * @see org.arquillian.cube.populator.api.Populator
     */
    private Optional<Annotation> getPopulatorAnnotation(Annotation... qualifiers) {
        return Arrays.stream(qualifiers)
                .filter(annotation -> annotation.annotationType().getAnnotationsByType(org.arquillian.cube.populator.api.Populator.class) != null)
                .findFirst();
    }
